...
{
	if( l >= r) return ;
	int mid = (l + r) / 2;//划分区间，并且将左区间和右区间归并排好序
	Merge(l,mid);
	Merge(mid+1,r);
	int i = l,j = mid+1,k = l;//现在，以mid为边界，左右半个区间皆已排好序，归并两个区间 
	while(i <= mid && j <= r){ 
		if(a[i] <= a[j]){
			t[k++] = a[i++];
		}
			...
			t[k++] = a[j++];
			ans = (ans + (mid - i + 1) % mod) % mod;//对于i到Mid这Mid - i + 1个数字（假设它为j），每一个a[j]都与a[i]构成了一个逆序对
		}
	}
	while( i <= mid){
		t[k++] = a[i++];
	}
	while(j <= r){//对于每个j都已讨论完，每个j钱比他大的数都已计入ans，所以此时不用更新ans
		t[k++] = a[j++];
	}
	for(int i = l;i<=r;i++){//归并完成，重置数组 
		a[i] = t[i];
	}
}
...
inline void addDan(int u, int v, int w, int c) 
{
	addedge(u, v, w, c);
	addedge(v, u, 0, -c);
}
...
inline bool spfa()
{
	...
	while (!q.empty()) {
		...
		for (int i = head[cur]; ~i; i = ed[i].next) {
			int to = ed[i].to;
			if (ed[i].flow > 0 && cost[to] > cost[cur] + ed[i].cost) {//cur.cost > cur.flow表示这条路还未流满（和最大流一样）
				cost[to] = cost[cur] + ed[i].cost;//更新最短路（最小费用） 
				pre[to] = cur;//更新前驱结点 
				from[to] = i;//记录连接cur与i的连边 
				flow[to] = min(flow[cur], ed[i].flow);//更新路径流量(到达v点的水量取决于边剩余的容量和u点的水量) 
				if (!vis[to]) {
					vis[to] = 1;
					q.push(to);
				}
...
inline void MCMF()
{
	...
	while (spfa()) {
		...
		MinCost += flow[t] * cost[t]; //对应最小费用 
		while (cur != s) {//从t点回溯更新残量 
			ed[from[cur]].flow -= flow[t];//反向边减去流量
			ed[from[cur] ^ 1].flow += flow[t];//正向边加上流量
			cur = pre[cur];
		}
	}
}
